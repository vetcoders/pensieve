...
- **A window is never closed out from under the work it is holding.** A background sweep tidies away empty "Pensieve" launcher windows a fifth of a second after they appear, and it decided what was empty from its own window bookkeeping — which cannot see what a window is actually showing. Two windows were invisible to it: the one holding an unsaved draft recovered from a previous session (it has no file behind it, so it never stopped looking like an empty launcher) and the one still loading the document you left open at quit (the document was on its way, but had not arrived yet). Whichever of the two lost that race was closed. When it was the loading one, the window died before its document arrived, so Pensieve never registered that you had closed anything — and the next launch dutifully reopened the same document again, every time. The sweep now asks each window what it is holding, leaves anything with unsaved work or a pending restore alone, and still clears away launchers that really are empty.
- **The "reopen this on launch" record is used once.** It was read at start-up but never cleared, so a document that had once been frontmost was restored on every launch afterwards, including launches where it had been closed. The launch that uses the record now consumes it; the window that is showing the document writes it back, so quitting with that document frontmost still restores it.
