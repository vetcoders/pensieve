...
final class DocumentExportTests: XCTestCase {
  ...
  /// rectangle only, so a dark surface came out as a dark block floating on
  /// white: a bare paper frame around every page and a bare tail under the last
  /// one. Export is WYSIWYG against the pane, so the whole sheet carries the
  /// surface — in either half of the theme.
  @MainActor
  func testPDFExportPaintsTheWholeSheetWithTheReadingSurface() throws {
    let letter = NSSize(width: 612, height: 792)
    let document = try longExportDocument(skin: .paper)

    let dark = try exportPDF(document, paperSize: letter, appearance: .darkAqua)
    ...
    let light = try exportPDF(document, paperSize: letter, appearance: .aqua)
    ...
    for index in 0..<light.pageCount {
      ...
      for (name, color) in try sheetCorners(of: page) {
        ...
        // Warm paper, not the pane's plain white base: the sheet has to carry
        // the colour read back off the rendered document, which is the only
        // place the theme's own page background exists.
        XCTAssertGreaterThan(
          color.redComponent - color.blueComponent, 0.02,
          "page \(index + 1) corner \(name) is the plain base (\(color)), not the theme's surface"
        )
      }
    }
  }
  ...
  @MainActor
  private func longExportDocument(skin: ThemeManager.PreviewTheme) throws -> PreviewDocument {
    let markdown = (1...60)
  ...
  /// Exports through the real print pipeline under an explicitly pinned
  /// appearance — the same resolution `DocumentExport.exportAppearance()`
  /// performs for the preview pane, without depending on the machine's setting.
  @MainActor
  private func exportPDF(
    ...
    let written = expectation(description: "PDF written under \(name.rawValue)")
    var outcome: Result<Void, Error>?
    job.start { result in
      outcome = result
      written.fulfill()
