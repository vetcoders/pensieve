...
final class LauncherSweepKeepsLiveWorkTests: XCTestCase {
  ...
  // ── (b) the window still restoring its document survives the sweep ───────
  @MainActor
  func testSweepKeepsAWindowWhoseRestoreHasNotResolvedYet() throws {
    let harness = try Harness(self)
    harness.persistReopenRecord(for: harness.documentURL)

    // The restoring window is a launcher until its document reaches the
    // accessor. A second window already counts as content, so the sweep's
    // "close all launchers" arm is armed — this is the mirror race that loses
    // the document instead of the draft.
    ...
    XCTAssertFalse(
      ...
      "the sweep reaped the window mid-restore — the document never appeared, and because "
        + "the window died with an empty session the reopen record was never retired")
    XCTAssertFalse(harness.wasClosed(otherWindow))

    // Once the restore resolves, the document really is there.
    restoring.controller.start(restoringWorkspace: true)
    XCTAssertEqual(
      restoring.appState.documentSession.url?.standardizedFileURL,
      harness.documentURL.standardizedFileURL,
      "the protected window did not end up showing the document it was restoring")
  }
  ...
  private final class Harness {
    ...
    func persistReopenRecord(for url: URL) {
